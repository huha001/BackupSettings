...
#region globaldefinitions
        ...
        string pathname = "NOT_DEFINED";
#if (!TV)
        public string myxmlfilename = "NOT_DEFINED";
#endif
        ...
        //bool duplicaterename = false;
        bool duplicateRemoveGroup = false;
...
#if (TV)
        #region constructor
        public void ExportImport(ref ProgressBar myprogressBar)
        {
        ...
        public void MyLoadSettings()
        {
            try
            {
                ...
                if ((setting.Value != "NOT_DEFINED") && (setting.Value != ""))
                ...
                //Duplicates
                setting = layer.GetSetting("Backup_SettingsDuplicateDelete", "false");
                duplicatedelete = Convert.ToBoolean(setting.Value);

                setting = layer.GetSetting("Backup_SettingsDuplicateRename", "true");
                duplicateRemoveGroup = Convert.ToBoolean(setting.Value);

                setting = layer.GetSetting("Backup_SettingsDuplicateRemoveGroup", "true");
                ...
                if (setting.Value != "")
                {
                    ...
                    for (int i = 0; i <= PB_PARTNUMBERS; i++)
                    {
                        ...
                        {
                            textoutput("<RED>Error: PB_Export Conversion " + i.ToString() + " failed");
                        }
                    }
                }
                //not used for export
                ...
                if (setting.Value != "")
                {
                    ...
                    { 
                        ...
                        {
                            textoutput("<RED>Error: PB_Import Conversion " + i.ToString() + " failed");
                        }
                    }
                }
        ...
        public string load_longsettings(string name)
        {
            ...
            //textoutput("Longsetting TvWishList_ListView");
            string stringdata = layer.GetSetting(name, "").Value;
            //textoutput("listviewdata TvWishList_ListView=" + listviewdata);
            int count = 1;
            string partial = layer.GetSetting(name + "_" + count.ToString("D3"), "").Value;
            //textoutput("partial " + "TvWishList_ListView_" + count.ToString("D3") + "=" + partial);
            while (partial != "")
            {
                stringdata += partial;
                count++;
            ...
            //textoutput("Merged" + "TvWishList_ListView Length =" + listviewdata.Length.ToString());
            //textoutput("Merged" + "TvWishList_ListView =" + listviewdata);
        ...
        {
            MessageBox.Show("a1");
            MessageBox.Show(myxmlfilename);
            MessageBox.Show("a1.5");
            if (File.Exists(myxmlfilename) == false)
            {
                MessageBox.Show("a1.6");
                textoutput("xml setting file ("+myxmlfilename+") not found - using defaultvalues");
                MessageBox.Show("a1.7");
                return;
            }
            MessageBox.Show("a2");
            try
        ...
        public bool createbackup(string folderpath)
        {
            ...
            if (TV) //19 criteria
            {
                ...
                defaultcheck = defaultcheck && TVallPrograms && TVAllFolders && TVRestart;
            }
            ...
            if (!defaultcheck)
            {
                switch (myMessageBox("You are not using the default filter settings for the backup - Only partial data will be exported.\n\n Do you want to abort? Yes is recommended", "Info: Default filter settings not used", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1))
                {
                    ...
                    case DialogResult.Yes:
                        {
...
#if PB
                                textoutput("progressBar.Maximum changed to " + progressBar.Maximum.ToString());
#endif
                            progressbar((int)PB_action.START, ref PB_Export, (int)PB_part.DeleteOldFiles);

                            try//delete Version.xml file
                            {
                            ...
                            {
                                textoutput("<RED>Could not delete file " + folderpath + @"\Version.xml - Exception: " + exc.Message);
                                return false;
                            }
                            try
                            {
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\TV_User - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\TV_Program - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\MP_User - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\MP_Program - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\SV2_User - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\SV2_Program - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\MP2_User - Exception: " + exc.Message);
                                return false;
                            }
                            ...
                            {
                                textoutput("<RED>Could not delete folder " + folderpath + @"\MP2_Program - Exception: " + exc.Message);
                                return false;
                            }
            ...
            else
            {
                ...
                if (File.Exists(TV_USER_FOLDER + "\\log\\tv.log") == false)
                {
            ...
            if (MP == false)
            {
                textoutput("Media Portal not checked - will not backup settings");
            }
            else
            {
                ...
                //---------------------------------
                //begin copy for BackupSettingsMP
                //---------------------------------
            ...
            if (MP2S == false)
            {
                textoutput("Media Portal2 Server not checked - will not backup settings");
            }
            else
            {
                ...
                if (MP2Defaults == true)
                {
                    ...
                    try
                    {
                        if (Directory.Exists(instpaths.DIR_SV2_Data + @"\Defaults") == true)
                        {
                            DirectoryCopy(instpaths.DIR_SV2_Data + @"\Defaults", folderpath + @"\SV2_PROGRAM\Defaults", "*", true, DEBUG, true); //overwrite verbose recursive
                        }
                    }
                    catch (Exception exc)
                    {
                        textoutput("<RED>Could not copy Defaults folder" + instpaths.DIR_SV2_Data + @"\Defaults" + " - Exception: " + exc.Message);
                        return false;
                    }
            ...
            if (MP2C == false)
            {
                textoutput("Media Portal2 Client not checked - will not backup settings");
            }
            else
            {
                ...
                if (MP2Defaults == true)
                {
                    ...
                    try
                    {
                        if (Directory.Exists(instpaths.DIR_MP2_Data + @"\Defaults") == true)
                        {
                            DirectoryCopy(instpaths.DIR_MP2_Data + @"\Defaults", folderpath + @"\MP2_PROGRAM\Defaults", "*", true, DEBUG, true); //overwrite verbose recursive
                        }
                    }
                    catch (Exception exc)
                    {
                        textoutput("<RED>Could not copy Defaults folder" + instpaths.DIR_MP2_Data + @"\Defaults" + " - Exception: " + exc.Message);
                        return false;
                    }
...
#if (MP12)
#else                       
                            AddAttribute(nodeRecording, "ChannelName", tmpchannel.Name);
#endif
        ...
        private void backupscripts(string folderpath, int number)
        {
            string scriptfile = "";
            string scriptname = "";
            switch (number)
        ...
        public bool restorebackup(string pathfolder)
        {
            ...
            else
            {
                ...
                if (File.Exists(TV_USER_FOLDER + "\\log\\tv.log") == false)
                {
                    textoutput("TV server data folder does not exist - aborting import");
                    return false;
                }
                //program data after user data to avoid reinitialization issue
                if (File.Exists(TV_PROGRAM_FOLDER + "\\TvService.exe") == false)
                ...
                //MySaveSettings();  needs to be done before importing class
            ...
            //bool okflag = completeimportxml(); // if needed stop tv server configuration and start post processing
                ...
                for (j = i + 1; j < channellist.Count; j++)
                {
...
#endif
                    {
                        if (firstfound == true)
                        {
                            if (verbose)
                            {
                                textoutput("<YELLOW>Warning: Duplicate channel names found for channel " + channelarray[i].DisplayName);
                                textoutput("<YELLOW>ID = " + channelarray[i].IdChannel);
                                firstfound = false;
                            }
                            else
                            {
                        ...
                        if (verbose)
                            textoutput("<YELLOW>Duplicate ID = " + channelarray[j].IdChannel);
            ...
            for (i = 0; i < channellist.Count - 1; i++)
            {
                ...
                for (j = i + 1; j < channellist.Count; j++)
                {
                    ...
                    {//channel name and MP display name do match
                        ...
                        {
                            if ((duplicateinteractive == true) && (duplicateRemoveGroup == true))
                            {
                            ...
                            else if (duplicateRemoveGroup == true)
                            {
                                textoutput("Renaming duplicate channel " + channelarray[j].DisplayName + " to " + channelarray[j].DisplayName + "_" + count.ToString());
                                channelarray[j].DisplayName = channelarray[j].DisplayName + "_" + count.ToString();
                                channelarray[j].Persist();
                ...
                if (recordings == true)
                {
                    ...
                    foreach (XmlNode nodeRecording in recordingList)
                    {
                        ...
                        try
                        {
                            ...
                            string displayname = nodeRecording.Attributes["DisplayName"].Value;
                            Channel tmpchannel = mygetChannelbyName(displayname);
                            if (tmpchannel == null)
                            {
                            ...
                            /*if (DEBUG)
                                Log.Info("Added recording: {0} for file: {1}", title, fileName);*/
                        }
                        catch (Exception ex)
                        {
                            recordingCount--;
                            textoutput("<RED>Could not create recording " + idRecording + " for title " + title + " and file " + fileName);
                            if (DEBUG)
                            {
                                textoutput("<RED>Exception message is " + ex.Message);
                            }
        ...
        public bool completeimportxml()
        {
            ...
            if (TVRestart == false)
            {
                ...
                textoutput("<YELLOW>Some settings will not get imported");
                POSTIMPORT = "";
                return true;
            }
            ...
            if (File.Exists(TV_PROGRAM_FOLDER + @"\" + BACKUPSETTING_TV_DIR + @"\" + POSTPROCESSING_FILE) == true)
            {
                File.Delete(TV_PROGRAM_FOLDER + @"\" + BACKUPSETTING_TV_DIR + @"\" + POSTPROCESSING_FILE);
                textoutput("BackupSettings Server: File " + TV_PROGRAM_FOLDER + @"\" + BACKUPSETTING_TV_DIR + @"\" + POSTPROCESSING_FILE + " deleted");
            }
            string fName = TV_PROGRAM_FOLDER + @"\" + BACKUPSETTING_TV_DIR + @"\" + POSTPROCESSING_FILE;
            try
            {
            ...
            appstartinfo.Arguments = "\"" + TV_PROGRAM_FOLDER + "\" POSTPROCESSING";
            ...
            Status.WriteLine("ProgressBar.Maximum  " + exportprogressBar.Maximum.ToString());
            Status.WriteLine("ProgressBar.Value  " + exportprogressBar.Maximum.ToString());//complete 100%
            ...
            textoutput("BackupSettings: Fatal Error: Running after application exit");
            return false;
        }
        ...
        public string CreateAutomatedFolderName(string folderpath)
        {
